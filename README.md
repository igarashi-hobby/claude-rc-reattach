...
| `save` | 切り替え**前** | RC 接続中の会話を manifest ファイルに記録し、`$STATE_DIR/recaps/<sessionId>.md` に事前あらすじを生成（一時ファイル → `mv` の原子的置換。ディレクトリ作成・書き込みに失敗した場合は「保存しました」と言わずエラーで終了し、既存の manifest も壊しません。switch はこの失敗で logout に進みません） |
| `restore` | 切り替え**後**（ログイン済み） | manifest の各会話を tmux 内で resume し、RC 再登録と登録確認まで行う。復元後は保存済みあらすじを表示し、既定ではセッションへ何も送信しない |
| `switch` | — | save → logout → login → restore を対話形式で一括実行 |
| `recover` | save し損ねた時 | manifest を使わず、直近に更新された会話ログから復旧候補を探して resume。保存済みあらすじがあれば表示 |
| `digest` | いつでも | manifest 全行（または名前フィルタ一致分）の保存済みあらすじをまとめて表示 |
...
1. **save** — RC 接続中の会話を manifest に記録し、あらすじを事前生成
...
| `--recap` | 旧方式: RC 登録できた会話に要約依頼を 1 通送る（トークン消費大・非推奨。既定OFF） |
| `--no-recap` | 互換用。既定OFFのため指定しても動作は変わりません |
...
#### 代わりに: save 時にあらすじを事前生成する

抑止されるのは**過去ログの一括アップロードだけ**で、RC 確立後の新しいやりとりは通常どおり双方向に同期されます。
ただし復帰後に各セッションへ「要約して」と送ると、開いている全セッションで推論が走り、アカウント切替のたびに高額なトークン消費が発生します。
そこで現在は、`save` 時に安価モデル（`claude -p --model haiku`）であらすじを事前生成し、`restore` / `switch` / `recover` 完了時は保存済み Markdown をターミナルに表示するだけにしています。
復元後のセッションへは既定で何も送信しません。

```bash
claude-rc-reattach save
# $STATE_DIR/recaps/<sessionId>.md にあらすじを事前生成

claude-rc-reattach restore
# 保存済みあらすじを表示。RC セッションへの要約依頼は送信しません

claude-rc-reattach digest
# manifest 全行のあらすじをまとめて表示
```

あらすじ生成は増分式です。
`$STATE_DIR/recaps/<sessionId>.state` に前回処理済みの行数とファイルハッシュを保存し、トランスクリプトに増分が無ければ前回の `.md` を維持します。
生成に失敗しても `save` は失敗させず、最後のユーザー発言・最後のアシスタント発言の先頭だけを機械整形したフォールバックを保存します。
コスト根拠: 旧方式は「復元後の全セッションが大型文脈で推論」でしたが、新方式は「save時に増分末尾最大8000字をHaikuへ1回」なので、切替後のAIトークン消費はほぼゼロです。

旧方式をどうしても使う場合だけ、`restore --recap` / `switch --recap` / `recover --recap` を付けてください。
これは RC 登録できた会話に要約依頼を 1 通送るため、**トークン消費大・非推奨**です。
`--no-recap` は互換用に受け付けますが、現在は既定OFFなので指定しても動作は変わりません。
`reconnect --recap` は従来どおり、今回再接続した会話にだけ旧方式の要約依頼を送ります。
旧方式のプロンプトは環境変数 `CLAUDE_RC_RECAP_PROMPT` で変更できます（送信後すぐ Enter を打つため、必ず 1 行で書いてください）。

名前で絞る場合:

```bash
claude-rc-reattach digest ミチガエル
```
...
ダイアログ待ちが残っていても、そこで打ち切らずに RC 登録の確認とあらすじ表示まで進めます。
...
| `CLAUDE_RC_RECAP_PROMPT` | `アカウント切替で画面上の履歴が空になっています。これまでの会話の経緯・決定事項・いま取り組んでいる作業を10行以内で要約して表示してください。` | 旧方式 `--recap` で送るプロンプト（必ず 1 行で書くこと） |
| `CLAUDE_RC_RECOVER_HOURS` | `6` | recover が対象とする時間（`--within` を付けた場合はそちらが優先） |
| `CLAUDE_RC_CONFIRM_WAIT` | `10` | recover で確認ダイアログを見に行くまでの待ち秒数 |
| `CLAUDE_RC_RECAP_MODEL` | `haiku` | save時のあらすじ生成に使うモデル（例: `sonnet`。軽い要約なのでhaiku推奨） |
...
- **リモート側には切り替え前のやりとりが表示されません。**
  ...
  復帰後の新しいやりとりは通常どおり同期されますが、既定ではトークン節約のため要約依頼を送らず、`save` 時に作ったあらすじをターミナルに表示します（[詳細](#リモート側に切り替え前の履歴は表示されない)）
...
| スマホ / claude.ai/code に切り替え前のやりとりが出ない | 仕様です（Claude Code のクロスアカウント抑止）。文脈確認は `restore` 完了時のあらすじ表示、または `claude-rc-reattach digest` を使ってください（上記「[リモート側に切り替え前の履歴は表示されない](#リモート側に切り替え前の履歴は表示されない)」参照） |
| `list` に `⚑ リモート履歴は抑止済み` と出る | その会話は既にクロスアカウント抑止が確定しており、以後どのアカウントで開いてもリモートに履歴は出ません。`save` 時のあらすじ、または `digest` で文脈を確認してください |
